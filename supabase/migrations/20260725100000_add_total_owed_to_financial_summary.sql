-- Exposes how much others owe the current user (previously computed
-- internally as `owed_total` but never returned) as `total_owed`, so the
-- home page can show it alongside debt and net balance.
DROP FUNCTION IF EXISTS "public"."get_user_financial_summary"();

CREATE FUNCTION "public"."get_user_financial_summary"()
RETURNS TABLE (
